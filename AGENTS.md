...
- **Per-task toolsets**: Each task can override which Hermes toolsets the agent loads (`toolsets` column on `tasks`, stored as a JSON array string or `null`). `GET /api/agent/toolsets` lists the available toolset names. When set, the worker passes the list as `enabled_toolsets` to `AIAgent`; when `null`, Hermes' own platform defaults apply.
